<?php
// public/admin/customers/quotation_edit.php — Quotation form (like invoice template): To, items table, Total, Discount, Process → Invoice
...
if (!function_exists('tt')) {
  function tt(string $key, string $fallback, array $params = []): string {
    return function_exists('t') ? (string)t($key, $params, $fallback) : $fallback;
  }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'process') {
    if ($id <= 0) {
      header('Location: ' . url('admin/customers/quotation_edit.php?customer_id=' . $cid));
      exit;
    }
    $st = $pdo->prepare("SELECT id, invoice_no, doc_flow_type FROM customer_txn WHERE id = :id AND customer_id = :cid LIMIT 1");
    $st->execute([':id' => $id, ':cid' => $cid]);
    $row = $st->fetch();
    if (!$row) {
      header('Location: ' . url('admin/customers/invoices.php?customer_id=' . $cid));
      exit;
    }
    ...
    header('Location: ' . url('admin/customers/txn_edit_in.php?id=' . $id . '&customer_id=' . $cid));
    exit;
  }
  ...
  if ($id > 0) {
    ...
    if ($hasSignReceive) { $sets[] = "sign_receive = :sign_receive"; $params[':sign_receive'] = $sign_receive; }
    if ($hasSignPayer) { $sets[] = "sign_payer = :sign_payer"; $params[':sign_payer'] = $sign_payer; }
    if ($hasRequireSignature) { $sets[] = "require_signature = :req_sig"; $params[':req_sig'] = ($sign_receive || $sign_payer) ? 1 : 0; }
    // QUOTATION 是否要客户签名：同步到 per-doc 字段，供 txn_doc_in.php 使用
    if ($hasRequireSignQuotation) { $sets[] = "require_sign_quotation = :req_sign_q"; $params[':req_sign_q'] = $sign_payer ? 1 : 0; }
    if ($hasDiscount) { $sets[] = "discount = :discount"; $params[':discount'] = $discount; }
    if ($hasDeliverTo) { $sets[] = "deliver_to = :deliver_to"; $params[':deliver_to'] = $deliver_to; }
    if ($hasTerms) { $sets[] = "terms = :terms"; $params[':terms'] = $terms; }
    if ($hasDoNumber) { $sets[] = "do_number = :do_number"; $params[':do_number'] = $do_number; }
    if ($hasSignMode) { $sets[] = "sign_mode = :sign_mode"; $params[':sign_mode'] = $sign_mode; }
    $sets[] = "notes = :notes";
    $params[':notes'] = $notes;
    $pdo->prepare("UPDATE customer_txn SET " . implode(', ', $sets) . " WHERE id = :id AND customer_id = :cid")->execute($params);
    ...
    if ($hasLines) {
      ...
      foreach ($linePosts as $row) {
        ...
        // strip existing marker then re-add if posted
        ...
        $pdo->prepare("INSERT INTO customer_txn_lines (customer_txn_id, line_seq, description, quantity, unit_price, amount) VALUES (:tid, :seq, :desc, :qty, :unit, :amt)")
          ->execute([':tid' => $id, ':seq' => $seq++, ':desc' => $desc, ':qty' => $qty, ':unit' => $unit, ':amt' => $amt]);
      }
    }
  } else {
  ...
  header('Location: ' . url('admin/customers/quotation_edit.php?id=' . $id . '&customer_id=' . $cid . '&ok=1'));
  exit;
}
...
<style>
.quotation-form table { width:100%; border-collapse:collapse; }
.quotation-form th, .quotation-form td { border:1px solid #e5e7eb; padding:8px; text-align:left; }
.quotation-form th { background:#f3f4f6; font-size:12px; }
.quotation-form .col-no { width:50px; }
.quotation-form .col-desc { min-width:200px; }
...
.quotation-form .col-qty { width:90px; }
.quotation-form .col-unit { width:110px; }
.quotation-form .col-amount { width:120px; text-align:right; }
.quotation-form .text-right { text-align:right; }
.quotation-form .doc-title { font-size:18px; font-weight:700; margin-bottom:12px; }
</style>
      ...
      <form method="post" class="quotation-form">
        ...
        <div class="form-grid form-grid-2" style="margin-bottom:16px;">
          <div>
            ...
            <div class="field-label" style="margin-bottom:4px;margin-top:8px;">Add.</div>
            ...
            <div class="field-label" style="margin-bottom:4px;margin-top:8px;">Tel</div>
            <input type="text" name="customer_tel" class="form-control" value="<?= h($customerTel) ?>" style="max-width:260px;">

            <div class="field-label" style="margin-bottom:4px;margin-top:8px;">Email</div>
            <input type="text" name="customer_email" class="form-control" value="<?= h($customerEmail) ?>" style="max-width:320px;">

            <div class="field-label" style="margin-bottom:4px;margin-top:8px;">Attn.</div>
          ...
          <div>
            ...
            <?php if ($hasDoNumber): ?>
              <div class="form-group">
                ...
                <input type="text" name="do_number" class="form-control" value="<?= h($txn['do_number'] ?? '') ?>" placeholder="Optional">
              </div>
            <?php endif; ?>
            <?php if ($hasDeliverTo): ?>
              <div class="form-group">
                <label class="field-label">Deliver To</label>
                <input type="text" name="deliver_to" class="form-control" value="<?= h($txn['deliver_to'] ?? '') ?>" placeholder="Address">
              </div>
            <?php endif; ?>
          </div>
        </div>
        ...
        <table class="table" style="margin-bottom:12px;">
          ...
          <tbody id="lineBody">
            ...
            <tr>
              ...
              <td><input type="number" step="0.0001" min="0" name="lines[<?= (int)$no ?>][quantity]" class="form-control line-qty" value="<?= h($qty) ?>"></td>
              <td>
                ...
                <input type="number" step="0.01" min="0" name="lines[<?= (int)$no ?>][unit_price]" class="form-control line-unit" value="<?= $unit > 0 ? h($unit) : '' ?>" placeholder="0.00">
              </td>
              <td><input type="number" step="0.01" min="0" name="lines[<?= (int)$no ?>][amount]" class="form-control line-amount text-right" value="<?= $amt > 0 ? h($amt) : '' ?>" placeholder=""></td>
            </tr>
            <?php
              }
            } elseif ($hasLines) {
              // 表存在但暂无行：显示一行空行，避免用 title/order_total 合并成「Quotation - 1600」
            ?>
            <tr>
              <td>1</td>
              <td><textarea name="lines[1][description]" class="form-control line-desc" rows="3" placeholder="Item description"></textarea></td>
            ...
            <?php } else {
              // 无 customer_txn_lines 表时的回退：单行用 title + order_total
              $savedTotal = (float)($txn['order_total'] ?? 0);
            ?>
            <tr>
              <td>1</td>
              <td><textarea name="lines[1][description]" class="form-control line-desc" rows="3" placeholder="Item description"><?= h($txn['title'] ?? '') ?></textarea></td>
              <td><input type="number" step="0.0001" min="0" name="lines[1][quantity]" class="form-control line-qty" value="1"></td>
              <td>
                <input type="text" name="lines[1][unit_label]" class="form-control line-unitlabel" value="" placeholder="SET / PAX" style="margin-bottom:6px;">
                <input type="number" step="0.01" min="0" name="lines[1][unit_price]" class="form-control line-unit" value="<?= $savedTotal > 0 ? h($savedTotal) : '' ?>" placeholder="0.00">
              </td>
              <td><input type="number" step="0.01" min="0" name="lines[1][amount]" class="form-control line-amount text-right" value="<?= $savedTotal > 0 ? h($savedTotal) : '' ?>" placeholder=""></td>
            </tr>
            <?php } ?>
          </tbody>
        </table>
        ...
        <?php if ($hasTerms): ?>
          <div class="form-group">
            ...
            <textarea name="terms" class="form-control" rows="3"><?= h($txn['terms'] ?? '') ?></textarea>
          </div>
        <?php endif; ?>
        <div class="form-group">
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:18px;">
          ...
          <?php endif; ?>
          <a href="<?= h(url('admin/customers/invoices.php?customer_id=' . $cid)) ?>" class="btn btn-light">Cancel</a>
        </div>
      </form>
    </div>
  </div>
...
(function(){
  ...
  function lineIndex() {
    return (lineBody.querySelectorAll('tr').length + 1);
  }
  ...
  function recalc() {
    ...
    var disc = parseFloat(totalDiscount.value) || 0;
    var grand = Math.max(0, total - disc);
    orderTotal.value = grand.toFixed(2);
  ...
  function bindLine(tr) {
    ...
    function updateAmount() {
      ...
      if (u > 0) {
        ...
        // unit price 为空时不自动覆盖 amount（允许手填 total）
        recalc();
      }
    }
