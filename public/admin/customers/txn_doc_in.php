...
.doc-table-items th, .doc-table-items td { border:0; padding:6px 8px; text-align:left; }
.doc-table-items { border:1px solid #333; }
.doc-table-items thead th { border-bottom:1px solid #333; }
.doc-table-items td, .doc-table-items th { border-right:1px solid #333; }
.doc-table-items td:last-child, .doc-table-items th:last-child { border-right:0; }
.doc-table-items tbody td { border-top:0; } /* 去掉行内横线 */
...
.doc-table-items th.col-qty, .doc-table-items td.col-qty { text-align:center; }
.doc-table-items th.col-unit, .doc-table-items td.col-unit { text-align:center; }
...
.print-header .h-row{ display:flex; gap:12px; align-items:flex-start; }
.print-header .h-logo img{ max-height:55px; }
.print-header .h-company{ font-size:11px; line-height:1.45; }
.print-header .h-company .name{ font-size:14px; font-weight:800; text-transform:uppercase; }
.print-header .h-meta{ margin-left:auto; text-align:right; font-size:12px; min-width:220px; }
.print-header .h-meta .title{ font-size:18px; font-weight:800; margin-bottom:6px; }
.print-header .h-to{ margin-top:10px; display:flex; justify-content:space-between; gap:18px; }
.print-header .h-to .block{ font-size:12px; line-height:1.55; }
.print-header .lbl{ font-weight:800; }

.print-footer{ font-size:11px; color:#374151; line-height:1.55; }
.print-footer .banks{ margin-top:8px; }
.print-footer .banks .row{ margin-top:2px; }
.print-footer .sig{ margin-top:10px; text-align:right; color:#111827; font-weight:800; }
.print-footer .sig-line{ margin-top:18px; border-top:1px solid #111827; width:220px; margin-left:auto; }
.print-footer .sig-cap{ font-size:11px; font-weight:700; margin-top:4px; text-align:right; color:#111827; }
...
@media print {
  ...
  #printPages, #printPages * { visibility:hidden; }
  #printPages { position:absolute; left:0; top:0; width:100%; }

  body.vm-print-ready .doc-print-wrap,
  body.vm-print-ready .doc-print-wrap * { visibility:hidden; }
  body.vm-print-ready #printPages,
  body.vm-print-ready #printPages * { visibility:visible; }
  ...
  /* 让银行区块两行尽量保持在同一页 */
  .doc-bottom-banks > div { break-inside: avoid; page-break-inside: avoid; }

  /* 打印时稍微压缩底部间距，减少提前分页 */
  .doc-bottom-terms { margin-top:12px !important; }
  .doc-bottom-banks { margin-top:8px !important; }
  .doc-sign-flex { margin-top:14px !important; }

  /* 打印分页版：每页 header/footer 固定，中间区域自动塞 items */
  #printPages { display:block !important; }
  /* 让分页版宽度跟原本单据一致（避免右边被裁切/标题被截断） */
  #printPages { left:0; top:0; }
  .print-page {
    width: 210mm;
    height: 297mm;
    margin: 0;
  ...
  .print-page .doc-table-items { width:100%; table-layout: fixed; }
  .print-page .doc-table-items th.col-desc { width:auto; }
  .print-page .doc-table-items td, .print-page .doc-table-items th { word-wrap:break-word; }
  /* 压缩表格行高/内边距，尽量一页放完 */
  .print-page .doc-table-items th,
  .print-page .doc-table-items td { padding: 4px 6px !important; }
  .print-page .doc-table-items th { font-size: 12px !important; }
  .print-page .doc-table-items td { font-size: 12px !important; }

  /* 压缩 footer（条款/银行/签名）高度，避免 chop 跑到下一页 */
  .print-page-footer { font-size: 10.5px !important; }
  .print-page-footer p { margin: 0 0 4px !important; }
  .print-page-footer .doc-bottom-terms { margin-top: 8px !important; }
  .print-page-footer .doc-bottom-banks { margin-top: 6px !important; }
  .print-page-footer .doc-sign-flex { margin-top: 10px !important; }
  .print-page-footer .sigimg { height: 64px !important; }
  .print-page-footer .sigpad-wrap { height: 64px !important; }
  .print-page-footer .doc-chop { max-height: 40px !important; }
  .print-page-footer .doc-underline { margin-top: 4px !important; }
  .print-page-footer .doc-sign-date-label { margin-top: 6px !important; }

  .print-page {
    page-break-after: always;
    break-after: page;
    position: relative;
    overflow: hidden; /* 关键：避免 footer(签名) 被推到下一页 */
  }
  .print-page:last-child { page-break-after: auto; break-after: auto; }
  .print-page-header { position:absolute; left:0; right:0; top: var(--hdrDrop, 0px); }
  .print-page-footer { position:absolute; left:0; right:0; bottom: var(--lift, 0px); margin-top:0; position: absolute; padding-bottom: 10px; }
  .print-page-no {
    position: absolute;
    left: 50%;
  ...
  .print-page-body { position:absolute; }
  .print-totals-wrap{
    position:absolute;
    left:0; right:0;
    bottom: var(--lift, 0px);
...
body.vm-print-build #printPages{
  ...
  position:absolute;
  left:-10000px; /* 屏幕上不打扰排版 */
  top:0;
  width: 210mm;
}
body.vm-print-build #doc-print-area{ display:none !important; }
...
body.vm-print-build .print-page-header{ position:absolute; left:0; right:0; top: var(--hdrDrop, 0px); }
body.vm-print-build .print-page-footer{ position:absolute; left:0; right:0; bottom: var(--lift, 0px); padding-bottom:10px; }
  ...
  <div style="display:flex; gap:18px; margin-top:10px; align-items:flex-end;">
    <?php if ($needCustomerSign): ?>
      <div style="flex:1;">
        ...
        <div style="font-weight:800; font-size:10.5px; margin-top:4px;">RECEIVED BY AND COMPANY STAMP:</div>
...
<div id="doc-print-area" class="doc-print-wrap">
  ...
  <div class="doc-sign-flex doc-bottom-sign">
    <div class="doc-sign-col" id="customerSignCol" style="<?= $needCustomerSign ? '' : 'display:none;' ?>">
      ...
      <div class="doc-sign-date-label" style="margin-top:12px;">DATE: <span class="sigdate" id="sigpadDateText"><?= h($customerSignDate) ?></span></div>
    </div>
    ...
    <div class="doc-sign-col right" id="companySignCol">
      <div style="font-weight:bold; margin-bottom:4px;"><?= h($companyName) ?></div>
        ...
        <div id="sigpadCompanyInput" style="<?= ($needCompanySign && $dbCompanySig === '') ? '' : 'display:none;' ?>">
...
(function(){
  ...
  function buildTotalsClone(docRoot){
    // totals 就是 table 后面那个 max-width:320px 的块
    // 给它加一个 class，避免靠结构猜太多
  ...
  function ensureTotalsClass(docRoot){
    ...
    for (var i=0;i<divs.length;i++){
      ...
      if (d && d.style && typeof d.style.maxWidth === 'string' && d.style.maxWidth.indexOf('320px') >= 0) {
        if (d.textContent && d.textContent.indexOf('Total Amount') >= 0) { candidate = d; break; }
      }
    }
  ...
  function makePage(headerClone, footerClone){
    ...
    pageNo.textContent = '';
    footer.appendChild(pageNo);

    page.appendChild(header);
    page.appendChild(body);
    page.appendChild(footer);

    return { page: page, body: body, itemsWrap: itemsWrap, totalsWrap: totalsWrap };
  }
  ...
  function paginate(){
    ...
    var rows = Array.prototype.slice.call(srcTbody.children).filter(function(n){ return n && n.tagName === 'TR'; });
    ...
    // 绿色区域固定容量（每页相同）：用“整页高度 - header - footer”计算，避免 clientHeight 受布局影响误判
    var pageH = Math.ceil(probe.page.offsetHeight || 0);
    var rawItemsMaxH = Math.max(0, pageH - (headerH + hdrDrop) - footerH);
    // 保守预留：宁可少放几行，也绝不让 footer/签名/chop 被挤出这一页
    // 但不能太大，否则会导致“本来一页放得下却被拆成两页”
    var safetyPad = 22; // px
    var itemsMaxH = Math.max(0, rawItemsMaxH - safetyPad);
    // 计算 totals 高度（最后一页要预留）
    var totalsH = 0;
    if (totalsClone) {
      probe.totalsWrap.appendChild(totalsClone);
      totalsH = Math.ceil(probe.totalsWrap.offsetHeight || 0);
      probe.totalsWrap.innerHTML = '';
    }
    ...
    function splitIntoPages(availableH){
      ...
      for (var i=0;i<rowHeights.length;i++){
        var rh = rowHeights[i];
        if (cur.length === 0 && used !== baseTableH) used = baseTableH;
        if (used + rh <= availableH) {
          cur.push(i);
          used += rh;
        } else {
          // 当前页放不下：开新页
    ...
    // 预留 totals：确保最后一页满足 itemsMaxH - totalsH
    var pagesIdx = normalPages.slice();
    if (totalsClone) {
      var allowedLast = Math.max(0, itemsMaxH - totalsH);

      // 如果全部 items + totals 本来就能放进一页，就强制一页完成（避免无意义的第二页）
      var totalRowsH = baseTableH;
      for (var ii=0; ii<rowHeights.length; ii++) totalRowsH += rowHeights[ii];
      // 用更贴近真实的“原始可用高度”做判断，避免因为 safetyPad 过保守导致拆页
      var liftLast = 26;
      var rawAllowedLast = Math.max(0, rawItemsMaxH - totalsH - liftLast);
      if (totalRowsH <= (rawAllowedLast + 8)) {
        var allIdx = [];
        for (var ai=0; ai<rowHeights.length; ai++) allIdx.push(ai);
        pagesIdx = [allIdx];
      }

      while (pagesIdx.length) {
        var lastPage = pagesIdx[pagesIdx.length - 1];
        // 计算 lastPage 的高度
        var lastH = baseTableH;
        for (var k=0;k<lastPage.length;k++) lastH += rowHeights[lastPage[k]];
        if (lastH <= allowedLast) break;
        // 不够：把最后一行挪到新的一页开头
        var moved = lastPage.pop();
        if (lastPage.length === 0) pagesIdx.pop();
        pagesIdx.push([moved]);
      }
    }

    // 一次性渲染所有页面
    // footer / totals 往上抬的距离（越大越靠上）
    // 你要缩小 table 下面的空白，所以把 footer 再往上抬
    var liftNormal = 52;
    var liftLast2 = 64;
    function renderPage(rowIndexList, isLast){
      ...
      p.page.style.setProperty('--totalsH', (isLast && totalsClone) ? (totalsH + 'px') : '0px');
      // 往上抬 footer（最后一页 totals+footer 一起抬更多一点）
      p.page.style.setProperty('--lift', (isLast ? liftLast2 : liftNormal) + 'px');
      ...
      if (isLast && totalsClone) {
        p.totalsWrap.appendChild(cloneNodeDeep(totalsClone));
      } else {
    ...
    pagesIdx.forEach(function(list, pi){
      var pageEl = renderPage(list, (pi === pagesIdx.length - 1));
    ...
    if (!pagesIdx.length) {
      ...
      emptyPage.page.style.setProperty('--totalsH', totalsClone ? (totalsH + 'px') : '0px');
      emptyPage.page.style.setProperty('--lift', (totalsClone ? liftLast2 : liftNormal) + 'px');
      var emptyTable = cloneTableSkeleton(srcTable);
      emptyPage.itemsWrap.appendChild(emptyTable);
      if (totalsClone) emptyPage.totalsWrap.appendChild(cloneNodeDeep(totalsClone)); else emptyPage.totalsWrap.style.display = 'none';
      var eno = qs('.print-page-no', emptyPage.page);
      if (eno) eno.textContent = 'Page 1/1';
      frag.appendChild(emptyPage.page);
    }
