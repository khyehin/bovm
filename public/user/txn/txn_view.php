...
/** ✅ Back URL：保留 list filters */
$backKeys = ['date_from', 'date_to', 'type', 'status', 'method', 'q'];
$backQ = [];
foreach ($backKeys as $k) {
  if (isset($_GET[$k]) && $_GET[$k] !== '') $backQ[$k] = (string)$_GET[$k];
}
$backUrl = url('user/txn/txns.php' . ($backQ ? ('?' . http_build_query($backQ)) : ''));
